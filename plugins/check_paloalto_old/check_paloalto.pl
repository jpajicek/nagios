...
} elsif ($check_type eq "session") {
   my $T_sess = $snmp_session->get_request(-varbindlist => [$s_tcp_sess]);
   my $tcpsess = "$T_sess->{$s_tcp_sess}";

   if($tcpsess > $crit) {
                $msg = "CRIT: TCP Sessions - $tcpsess";
                $stat = 2;
   } elsif($tcpsess > $warn) {
                $msg = "WARN: TCP Sessions - $tcpsess";
                $stat = 1;
   } else {
                $msg = "OK: TCP Sessions - $tcpsess";
                $stat = 0;
   }
   $perf = "tcp_session=$tcpsess;$warn;$crit";
